    ...
    registerTool<McpLinkIssuesDto>(
      ...
      'link_issues',
      {
        ...
        description:
          ...
          '"BUG-3 duplicate-of BUG-1". `type` accepts blocks, blocked-by, related-to or ' +
          'duplicate-of; the relation reads from `from` to `to`. An unknown type comes back with ' +
          'the valid choices. These are peer relations only — to nest one issue under another use ' +
          'update_issue with `parent` (an issue has exactly one parent). Use list_links to see or ' +
