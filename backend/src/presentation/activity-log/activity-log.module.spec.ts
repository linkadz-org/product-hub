 ...
 * IIssueRepository, IDocPageRepository, IRoadmapRepository and
 * IReportRepository (Task 17's related-history assembly) injected, and every
 * module in between exports what the next one needs — is to compile the real
 * module graph here with the Mongoose models it pulls in stubbed out. A stub
 * model is safe because the repository constructors only store the injected
 * model; they never call it until a query method runs.
 */
